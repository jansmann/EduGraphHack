                ...
                if (activity.Type == ActivityTypes.Message)
                {
                    if (activity.Text == "login")
                    {
                        ...
                        CardAction plButton = new CardAction()
                        {
                            ...
                            Value = $"https://login.microsoftonline.com/microsofteduo365.onmicrosoft.com/oauth2/authorize?client_id=59818122-3f7b-4017-abeb-5bdf5af2c95b&response_type=code&redirect_uri=http://localhost:8080/api/auth/&response_mode=query&scope=offline_access%20eduroster.readwrite%20eduassignments.readwrite&state={HttpUtility.UrlEncode(JsonConvert.SerializeObject(state))}",
                            Type = "signin",
